...
export default function Dashboard() {
    ...
    const [currentView, setCurrentView] = useState("meetings"); // "meetings" | "history" | "board" | "analytics" | "team"
    ...
    const [pastMeetingTasks, setPastMeetingTasks] = useState([]);
    ...
    useEffect(() => {
        if (user?.team) {
            ...
            socketRef.current.on("task-board-update", (data) => {
                fetchBoardTasks();
                if (currentView === "analytics") {
                    fetchAnalytics();
                }
    ...
    const handleJoin = (meetingId) => navigate(`/room/${meetingId}`);

    // ── Join meeting by link or ID ───────────────────────────────────────────
    ...
    const openPastMeetingDetails = async (meeting) => {
    ...
    const handleTogglePastTask = async (task) => {
    ...
    const handleAddBoardTask = async (e) => {
        e.preventDefault();
    ...
    const handleMoveBoardTask = async (task, targetStatus) => {
        try {
